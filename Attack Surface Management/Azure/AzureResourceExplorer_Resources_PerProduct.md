...
| summarize Count = count() by ProtectBucket, ResourceCategory
| order by ProtectBucket asc, ResourceCategory asc
| summarize ResourceSummary = strcat_array(make_list(strcat(Count, " ", ResourceCategory)), ", ")
    by ProtectBucket
| order by ProtectBucket asc
```

Per Subscription + Per Product + Other
```kql
Resources
| extend ResourceType = tolower(type)
| extend ResourceCategory = case(
